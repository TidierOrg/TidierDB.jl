...
function expr_to_sql_clickhouse(expr, sq; from_summarize::Bool)
    ...
    MacroTools.postwalk(expr) do x
        ...
        elseif @capture(x, strreplace(str_, pattern_, replace_))
            return :(replaceRegexpOne($str, $pattern, $replace))
        elseif @capture(x, strremoveall(str_, pattern_))
            return :(replaceRegexpAll($str, $pattern, ""))
        elseif @capture(x, strremove(str_, pattern_))
            return :(replaceRegexpOne($str, $pattern, ""))
        ...
        elseif @capture(x, mdy(time_))
            return :(formatDateTime($time, "MM-dd-yyyy"))
        elseif @capture(x, dmy(time_))
            return :(formatDateTime($time, "dd-MM-yyyy"))
